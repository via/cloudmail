...
class CloudFSImapMailbox(object):
  ...
  def fetch(self, messages, uid):
    ...
    for msg in messages:
      if msg >= self.getUIDNext():
        continue
      result.append( (msg, CloudFSImapMessage(self.user, self.folder, msg,
        self.pool)))
    return result
  ...
  def addMessage(self, msg, flags=None, date=None):
    ...
    self.conn.sadd("%s:mailboxes:%s:flags" % (self.user, self.folder),
      ["\Recent"])
    return defer.succeed(msg_uid)
  ...
  def store(self, messageSet, flags, mode, uid):
    ...
    for i in messageSet:
      if mode == -1:
        self.conn.srem("%s:mailboxes:%s:mail:%s:flags" % (self.user,
          self.folder, uid), *flags)
      elif mode == 0:
        pipe = self.conn.pipeline(transaction=True)
        pipe.multi()
        pipe.delete("%s:mailboxes:%s:mail:%s:flags" % (self.user,
          self.folder, uid))
        pipe.sadd("%s:mailboxes:%s:mail:%s:flags" % (self.user, self.folder,
          uid), *flags)
        pipe.execute()
      elif mode == 1:
        self.conn.sadd("%s:mailboxes:%s:mail:%s:flags" % (self.user, self.folder,
